...
def compute_patch_similarity(timestep_a, timestep_b, resolution, output_dir="output"):
    """
    ...
    """
    # --- Load FAISS indices ---
    path_a = os.path.join(output_dir, f"mfa_patches_{timestep_a}-{resolution}-approx.index")
    path_b = os.path.join(output_dir, f"mfa_patches_{timestep_b}-{resolution}-approx.index")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resolution", help='resolution (string), eg "32x32x64"')
    parser.add_argument("--output-dir", default="output",
                        help="Directory containing FAISS index files (default: output)")
    args = parser.parse_args()

    compute_patch_similarity(args.timestep_a, args.timestep_b, args.resolution, args.output_dir)
