...
int main(int argc, char** argv)
{
    ...
    int         vars_degree     = 3;        // degree for science variables (same for all dims)
    ...
    // following sizes are hard-coded for the argon bubble dataset
    ...
    {
        // last argument true means swap order to be able to use structured grid
        // argon bubble dataset is laid out in opposite order (z changing fastest)
        // than MFA expects (x changing fastest)
        b->read_3d_coords_and_scalar_data(cp, mfa_info, d_args, true);
    });
    ...
    // debug: print first 10 control points
//     master.foreach([&](Block<real_t>* b, const diy::Master::ProxyWithLink& cp)
//             { b->print_ctrl(cp); });
